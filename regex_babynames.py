...
def extractFemaleNames(filename,letter):
  ...
  text=f.read()
  list_name= []
  rtext=re.findall(r'<td>(\d+)</td><td>\w+</td><td>(\w+)</td>', text)
  for i in rtext:
    (rank, name)=i
    if name[0]==letter:
      list_name.append(name + " " + rank)
...
def extractMaleNames(filename,letter):
  ...
  text=f.read()
  list_name= []
  rtext=re.findall(r'<td>(\d+)</td><td>(\w+)</td><td>', text)
  for i in rtext:
    (rank, name)=i
    if name[0]==letter:
      list_name.append(name + " " + rank)
  return sorted(list_name, reverse=True)
...
def extractDocAndPopularityDate(filename):
  f=open(filename, 'r')
  text=f.read()
  rtext=re.search(r'(\w+\s\w+\W\s\d+)', text)
  ltext=re.search(r'(\w+\s\w+\s\d+)', text)
  return (rtext.group() + " and " + ltext.group())
